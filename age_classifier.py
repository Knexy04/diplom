...
def estimate_head_body_ratio_confidence(person: PersonDetection) -> float | None:
    ...
    # ratio: дети ~4.0-5.5, взрослые ~6.5-7.5+
    # Линейная шкала: 5.0 → 0.0 (child), 7.0 → 1.0 (adult)
    score = np.clip((ratio - 5.0) / 2.0, 0.0, 1.0)
    return float(score)
...
class EnsembleAgeClassifier(AgeClassifier):
    ...
    # Веса выровнены по фактической точности методов (абляция):
    # лицо 0.86 > YOLO 0.79 > head-body 0.76 > pose 0.74 > rel-height 0.70 > abs-height 0.66
    YOLO_WEIGHT = 2.5         # YOLO26 direct — стабильный голос, есть почти всегда
    POSE_WEIGHT = 1.5         # Пропорции скелета — было 1.0 (недооценён при точности 0.74)
    ML_WEIGHT = 2.5           # ML по лицу — самый точный (0.86), когда лицо видно; было 1.0
    HEAD_BODY_WEIGHT = 2.5    # Head-to-body ratio — работает в любой позе
    HEIGHT_REL_WEIGHT = 2.0   # Относительная высота (к другим в кадре) — было 3.0 (переоценён)
    HEIGHT_ABS_WEIGHT = 1.0   # Абсолютная высота — самый слабый метод (0.66); было 2.0
